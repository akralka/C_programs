...
int main() {
    ...
    int f, s;       // f - flaga(potrzebna jest) ,   s - pole powierzchni
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            ...
            while(i + a < n-1){
                ...
                b = 0;
                f = 1;
                int p;      // jedziemy po wierszach w nowych kolumnach
                while(f == 1 && j + b <= n-1){
                    for(p = 0; p <= a; p++){
                        if(T[i+p][j+b] == 1){
                            f = 0;      // ups, nieuzytkowy, wracaaamy
                            b -= 1;
                            break;
                        }
                    }
                    if(j+b == n-1){
                        f = 0;      // bez tego petla nie dziala
                    }
                    if(f == 1 && j + b < n-1){
